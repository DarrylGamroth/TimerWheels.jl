...
@testset "Performance Tests" begin
    ...
    @testset "Basic Operations" begin
        @testset "Schedule Timers" begin
            ...
            @printf(
                "%-25s | %8d | %10.3f ms | %8.1f ns\n",
                "Schedule",
                BENCHMARK_SIZE,
                schedule_time / 1_000_000,
                time_per_timer
            )
        end
        ...
        @testset "Cancel Timers" begin
            wheel = DeadlineTimerWheel(0, 1, 1024)
            timer_ids = [schedule_timer!(wheel, i * 100) for i = 1:BENCHMARK_SIZE]

            start_time = time_ns()
            cancelled_count = 0
            for i = 1:2:BENCHMARK_SIZE  # Cancel half
            ...
            @printf(
                ...
                "Cancel",
                cancelled_count,
                cancel_time / 1_000_000,
                time_per_cancellation
            )
        end
    end
    ...
    @testset "Memory and Expansion" begin
        @testset "Capacity Expansion" begin
            ...
            @printf(
                "%-25s | %8d | %10.3f ms | %8.1f ns\n",
                "Schedule (w/ expansion)",
                BENCHMARK_SUBSET,
                expansion_time / 1_000_000,
                time_per_timer
            )
        end
    ...
    @testset "Stress Tests" begin
        @testset "Mixed Operations" begin
            ...
            @printf(
                ...
                "Mixed operations",
                BENCHMARK_SIZE,
                stress_time / 1_000_000,
                time_per_operation
            )
            @printf(
                "%-25s | %8s | %10s | Final: %d active\n",
                "",
                "",
                "",
    ...
    @testset "Worst Case Scenarios" begin
        ...
        @testset "Large Time Range" begin
            ...
            poll_start = time_ns()
            poll_result =
                poll(wheel, large_deadline + 1000, nothing) do client, now, timer_id
                    expired_count += 1
                    return true
                end
            poll_time = time_ns() - poll_start
